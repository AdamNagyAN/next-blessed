...
export async function Page({ params }: PageProps) {
  ...
  const discountPercentage: number = newPrice ? 100 - (newPrice / price) * 100 : 0;
        ...
        <div className="grid grid-cols-2 justify-items-center">
          ...
          <div className="w-full">
            ...
            <span>
              <span
                className={cn({
                  'line-through mr-2': !!newPrice && discountPercentage > 0
                })}>{`${price} ${siteConfig.currency}`}</span>
              {newPrice && discountPercentage > 0 && (
                <span className="text-destructive font-semibold">{formatPrice(newPrice)}</span>
              )}
            </span>
            <div className="mt-6">
